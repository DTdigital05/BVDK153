...
import { 
  ...
  MessageCircle, Send, Minimize2, Upload, CheckCircle, Star, ChevronLeft, Pencil, ClipboardList
} from 'lucide-react';
...
const App = () => {
  ...
  const [activeAdminTab, setActiveAdminTab] = useState('bookings'); // Mặc định vào xem lịch đặt
  const [loading, setLoading] = useState(true);
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  // --- STATE CHO HERO SLIDER ---
  ...
  const [bookings, setBookings] = useState([]); // Danh sách đặt lịch
  ...
  const [heroSlides, setHeroSlides] = useState([
    {
      ...
      img: "https://images.unsplash.com/photo-1631217868264-e5b90bb7e133?auto=format&fit=crop&q=80&w=1200"
    },
    {
      id: 2,
      title1: "Công Nghệ Cao",
      title2: "Chẩn Đoán Chính Xác",
      desc: "Ứng dụng các kỹ thuật chẩn đoán hình ảnh tiên tiến: CT-Scanner, MRI, Nội soi NBI giúp phát hiện bệnh sớm.",
      img: "https://images.unsplash.com/photo-1579684385180-1ea55f9f8981?auto=format&fit=crop&q=80&w=1200"
    },
    {
      id: 3,
      title1: "Đội Ngũ Chuyên Gia",
      title2: "Tận Tâm - Y Đức",
      desc: "Quy tụ đội ngũ bác sĩ chuyên khoa giàu kinh nghiệm từ các bệnh viện tuyến trung ương.",
  ...
  // State để quản lý việc sửa tin tức
    ...
    if (db) {
      try {
        const [docSnap, specSnap, pkgSnap, newsSnap, bookSnap] = await Promise.all([
          ...
          getDocs(collection(db, "bookings")) // Lấy danh sách đặt lịch
        ]);
      ...
      setBookings([
        { id: 1, name: "Nguyễn Văn Demo", phone: "0987654321", date: "2024-06-20", doctor: "BS.CKII Nguyễn Văn A", note: "Đau đầu, chóng mặt", createdAt: new Date().toISOString() }
      ]);
    }
    setLoading(false);
  };

  // --- HÀM SCROLL TO SECTION ---
  ...
  const updateItem = async (col, id, item, setSt, st) => {
    if (db) { await updateDoc(doc(db, col, id), item); fetchData(); } 
    else { setSt(st.map(i => i.id === id ? { ...item, id } : i)); }
  };
  ...
  // Helper để xử lý việc Sửa Tin Tức
  ...
  const submitBooking = async (e) => { 
    ...
    if (db) {
      ...
      fetchData(); // Cập nhật lại list cho Admin
    } else {
    ...
    <div className="min-h-screen bg-slate-50 flex">
      <aside className="w-64 bg-slate-900 text-white flex flex-col fixed h-full z-10 overflow-y-auto">
        ...
        <nav className="flex-1 p-4 space-y-2">
          {/* Menu Item Mới: Quản Lý Đặt Lịch */}
          <button onClick={()=>setActiveAdminTab('bookings')} className={`w-full text-left p-3 rounded capitalize flex gap-2 items-center ${activeAdminTab==='bookings'?'bg-blue-600':'hover:bg-slate-800'}`}>
            <ClipboardList size={18}/> Đặt Lịch Khám
          </button>
      ...
      <main className="ml-64 flex-1 p-8 bg-slate-100 min-h-screen">
        ...
        {/* TAB: QUẢN LÝ ĐẶT LỊCH (MỚI) */}
        {activeAdminTab === 'bookings' && (
          <div className="max-w-6xl mx-auto">
            <h2 className="text-2xl font-bold mb-6 text-slate-800 flex items-center gap-2"><ClipboardList/> Danh Sách Khách Đăng Ký</h2>
                ...
                <table className="w-full text-sm text-left">
                  <thead className="bg-slate-50 text-slate-700 uppercase font-bold">
                    <tr>
                      ...
                      <th className="p-4">Yêu cầu / Ghi chú</th>
                  ...
                  <tbody className="divide-y">
                    {bookings.map(b => (
                      <tr key={b.id} className="hover:bg-slate-50">
                        ...
                        <td className="p-4 font-bold text-blue-700">{b.name}</td>
                        <td className="p-4">{b.phone}</td>
                        <td className="p-4 max-w-xs">
                          <div><span className="font-bold">Khám:</span> {b.doctor || b.date || 'Chưa rõ'}</div>
                          <div className="text-slate-500 italic truncate">{b.note}</div>
                        </td>
                        <td className="p-4">
                          <button onClick={()=>deleteItem('bookings', b.id, setBookings, bookings)} className="text-red-500 hover:bg-red-100 p-2 rounded" title="Xóa sau khi đã gọi"><Trash2 size={18}/></button>
                        </td>
                      </tr>
                    ))}
                    {bookings.length === 0 && <tr><td colSpan="5" className="p-8 text-center text-slate-500">Chưa có ai đăng ký.</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
        ...
        {activeAdminTab === 'doctors' && (
          <div className="max-w-4xl mx-auto"><h2 className="text-2xl font-bold mb-6 text-slate-800">Quản Lý Bác Sĩ</h2>
            <div className="bg-white p-6 rounded-lg shadow mb-8"><h3 className="font-bold mb-4 text-blue-600">Thêm Bác Sĩ Mới</h3><div className="grid gap-4"><div className="grid grid-cols-2 gap-4"><input placeholder="Họ tên" className="border p-2 rounded" value={newDoctor.name} onChange={e=>setNewDoctor({...newDoctor, name:e.target.value})}/><input placeholder="Chức vụ" className="border p-2 rounded" value={newDoctor.role} onChange={e=>setNewDoctor({...newDoctor, role:e.target.value})}/></div><textarea placeholder="Tiểu sử / Giới thiệu chi tiết" className="border p-2 rounded" rows={3} value={newDoctor.bio} onChange={e=>setNewDoctor({...newDoctor, bio:e.target.value})}/><div className="flex items-center gap-3"><label className="cursor-pointer bg-slate-100 hover:bg-slate-200 px-3 py-2 rounded flex items-center gap-2"><Upload size={16}/> Ảnh đại diện <input type="file" className="hidden" onChange={e=>handleImageUpload(e, u=>setNewDoctor({...newDoctor, img:u}))}/></label>{newDoctor.img && <img src={newDoctor.img} className="h-10 w-10 rounded-full object-cover"/>}</div><button onClick={()=>{addItem('doctors', newDoctor, setDoctors, doctors); setNewDoctor({name:'',role:'',img:'',bio:''})}} className="bg-blue-600 text-white p-2 rounded font-bold hover:bg-blue-700">Lưu Bác Sĩ</button></div></div>
  ...
  return (
    <div className="font-sans text-slate-600 antialiased bg-white relative">
      ...
      <header className="sticky top-0 z-40 bg-white py-4 shadow-sm">
        <div className="container mx-auto px-4 flex justify-between items-center">
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => scrollToSection(homeRef)}>{renderLogo()}<div><span className="font-bold text-slate-900 text-lg block leading-none">{generalInfo.siteName}</span><span className="text-[10px] font-bold text-blue-600 uppercase">{generalInfo.siteSubName}</span></div></div>
          <nav className="hidden lg:flex gap-8 font-medium">
            <button onClick={() => scrollToSection(homeRef)} className="hover:text-blue-700 py-2">Trang chủ</button>
            <button onClick={() => scrollToSection(aboutRef)} className="hover:text-blue-700 py-2">Giới thiệu</button>
            <button onClick={() => scrollToSection(specialtiesRef)} className="hover:text-blue-700 py-2">Chuyên khoa</button>
            <button onClick={() => scrollToSection(packagesRef)} className="hover:text-blue-700 py-2">Gói khám</button>
            <button onClick={() => scrollToSection(doctorsRef)} className="hover:text-blue-700 py-2">Bác sĩ</button>
            <button onClick={() => scrollToSection(newsRef)} className="hover:text-blue-700 py-2">Tin tức</button>
          </nav>
        ...
        {isMenuOpen && <div className="lg:hidden bg-white border-t p-4 flex flex-col gap-4">
          <button onClick={() => scrollToSection(homeRef)} className="font-bold text-left">Trang chủ</button>
          <button onClick={() => scrollToSection(aboutRef)} className="font-bold text-left">Giới thiệu</button>
          <button onClick={() => scrollToSection(specialtiesRef)} className="font-bold text-left">Chuyên khoa</button>
          <button onClick={() => scrollToSection(packagesRef)} className="font-bold text-left">Gói khám</button>
          <button onClick={() => scrollToSection(doctorsRef)} className="font-bold text-left">Bác sĩ</button>
          <button onClick={() => scrollToSection(newsRef)} className="font-bold text-left">Tin tức</button>
        </div>}
      </header>

      {/* 2. Hero SLIDER */}
      <section ref={homeRef} className="pt-8 pb-12 bg-slate-50 overflow-hidden relative min-h-[500px] flex items-center">
        {heroSlides.map((slide, index) => (
          <div key={slide.id} className={`absolute inset-0 transition-opacity duration-1000 ${index === currentSlide ? 'opacity-100' : 'opacity-0'}`}>
            <div className="absolute top-0 right-0 w-2/3 h-full bg-cover bg-no-repeat opacity-20 rounded-bl-[200px]" style={{ backgroundImage: `url(${slide.img})`, backgroundPosition: 'center' }}></div>
          </div>
        ))}
        <div className="container mx-auto px-4 grid lg:grid-cols-2 gap-12 items-center relative z-10">
          <div className="space-y-6 animate-in slide-in-from-left duration-700 fade-in" key={currentSlide}>
            ...
            <h1 className="text-4xl lg:text-6xl font-extrabold text-slate-900 leading-tight">{heroSlides[currentSlide].title1} <br/><span className="text-blue-700">{heroSlides[currentSlide].title2}</span></h1>
            <p className="text-lg text-slate-600">{heroSlides[currentSlide].desc}</p>
          ...
          <div className="relative h-[400px]">
             {heroSlides.map((slide, index) => (<img key={slide.id} src={slide.img} className={`absolute top-0 left-0 w-full h-full object-cover rounded-3xl shadow-2xl border-4 border-white transition-opacity duration-1000 ${index === currentSlide ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`} alt="Slide" />))}
             <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">{heroSlides.map((_, idx) => (<button key={idx} onClick={() => setCurrentSlide(idx)} className={`w-3 h-3 rounded-full transition-all ${idx === currentSlide ? 'bg-blue-600 w-8' : 'bg-white/50 hover:bg-white'}`}/>))}</div>
          </div>
        </div>
      </section>
