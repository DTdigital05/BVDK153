...
const App = () => {
  ...
  // --- HÀM XỬ LÝ ẢNH (Tự động nén xuống < 500KB) ---
  const handleImageUpload = (e, cb) => {
    const file = e.target.files[0];
    if (!file) return;

    // Helper: Nén ảnh dùng Canvas
    const compressImage = (sourceStr, quality = 0.7, maxWidth = 1000) => {
    ...
    reader.onload = async (event) => {
      ...
      // Nếu file > 500KB thì bắt đầu nén
      if (file.size > 500 * 1024) {
        // Nén lần 1: Resize 1000px, chất lượng 70%
        result = await compressImage(result, 0.7, 1000);
        
        // Kiểm tra sơ bộ kích thước base64 (Base64 dài ~1.33 lần binary)
        // 500KB ~ 680,000 ký tự base64
        if (result.length > 700000) {
           // Nén lần 2 mạnh hơn nếu vẫn to: Resize 800px, chất lượng 50%
           result = await compressImage(result, 0.5, 800); 
        }
      }
      cb(result);
    };
